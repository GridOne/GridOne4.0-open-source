...
package kr.co.actsone.controls.listClasses
{
	...
	import mx.controls.listClasses.ListBaseFindPending;
	...
	import mx.controls.listClasses.ListBaseSelectionPending;
	 ...
	 *  When an ExAdvancedListBase-derived component is a potential drop target in a
	...
	/**
	 *  The ExAdvancedListBase class is the base class for controls,
	 ...
	 *
	 *  <p>In a model/view architecture, the ExAdvancedListBase subclass represent
	 *  the view, and the data provider represents the model.</p>
	 *
	 *  @mxml
	 *  
	 *  <p>The ExAdvancedListBase class inherits all of the tag properties of its superclasses,
	 ...
	 *  @productversion Flex 3
	 */
	public class ExAdvancedListBase extends ScrollControlBase
		...
		IEffectTargetHost
	{
		 ...
		 *  @productversion Flex 3
		 */
		public function ExAdvancedListBase()
		{
			super();
		 ...
		 *  The ExAdvancedListBase class then uses this property to access
		 ...
		 *  @productversion Flex 3
		 */
		protected var listContent:ExAdvancedListBaseContentHolder;
		...
		// use for bExternalScroll=true
		//by chheavhun 2013/10/17
		///////////////////////////////////////////////
		public var bscrollStart:Boolean=false;
		...
		override public function set verticalScrollPosition(value:Number):void
		{
			...
			if (oldValue != value)
			{
				...
				{
					bscrollStart=true;
					//trace(" deltaPos "+ deltaPos +" (rowInfo.length - lockedRowCount) "+(rowInfo.length - lockedRowCount));
		 ...
		 *  For example, the ExAdvancedDataGrid class uses 
		 *  ExAdvancedDataGridItemRenderer.</p>
		 *  
		 *  @langversion 3.0
		 *  @playerversion Flash 9
		 *  @playerversion AIR 1.1
		...
		public var _selectedIndices:Array;
		...
		{
			//if (ExAdvancedListBase.createAccessibilityImplementation != null)
			//ExAdvancedListBase.createAccessibilityImplementation(this);
		}
		...
		override protected function createChildren():void
		{
			...
			if (!listContent)
			{
				listContent = new ExAdvancedListBaseContentHolder(this);
													  ...
													  unscaledHeight:Number):void
		{
			...
			{
				g.beginFill(0x808080, 0);//0x808080
			...
			else    // redo all layout
			{
				...
				if (iterator)
				{
					if (offscreenExtraColumns)
				 	...
				 	else if (!bscrollStart) //chheavhun add for scroll performance
		 ...
		 *  same size.
		 *
		 *  <p>This method is not implemented in the ExAdvancedListBase class
		...
		protected function itemRendererToIndices(item:IListItemRenderer):Point
		{
			...
			//Duong modify to prevent error to be occurred in case index=-1
			if(index == -1)
				return null;
									...
									transition:Boolean = false):void
		{
			...
			if (selected)
			{
				var effectiveRowY:Number = this.runningDataEffect ? 
...
//				listContent.addChild(mask);
		 ...
		 *
		 *  <p>Not implemented in ExAdvancedListBase because the default list
											...
											scrollUp:Boolean):void
		{
			...
			if (scrollUp)
			{
				...
				if (deltaPos)
				{
					...
					//Long add on 2013/01/ on 2013/01/30
					listItems = cutArray(listItems, 0,rowCount - deltaPos);
					rowInfo = cutArray(rowInfo, 0, rowCount - deltaPos);
			...
			else
			{
				...
				if (actual.y == 0)
				{
					...
					//Long add on 2013/01/ on 2013/01/30
					var a1:Array =cutArray(listItems, 0,lockedRowCount);
					var a2:Array = cutArray(listItems, lockedRowCount+deltaPos,listItems.length);
					listItems = a1.concat(a2);
					
					a1 =cutArray(rowInfo, 0,lockedRowCount);
					a2 = cutArray(rowInfo, lockedRowCount+deltaPos,rowInfo.length);
					rowInfo = a1.concat(a2);
				...
				for (i = lockedRowCount + actual.y; i < listItems.length; i++)
				{
					...
					if (deleteRow)
					{
						...
						//Long add on 2013/01/ on 2013/01/30
						a1 =cutArray(listItems, 0,i);
						a2 = cutArray(listItems, i+1,listItems.length);
						listItems = a1.concat(a2);
						
						a1 =cutArray(rowInfo, 0,i);
						a2 = cutArray(rowInfo, i+1,rowInfo.length);
						rowInfo = a1.concat(a2);
		...
		/**********************************************************************************/
		/**  Long created on 2013/01/30 to replaced splice function						***/
		/**********************************************************************************/
		private function cutArray(inputArr:Array, startIndex:int,removedIndex:int):Array
		{
			var outputArr:Array = [];
			var index:int = 0;
			for(var i:int=startIndex;i<inputArr.length;i++)
			{
				if(i<removedIndex)
		 ...
		 *  for automation delegate access
		 */
		mx_internal function getListContentHolder():ExAdvancedListBaseContentHolder
		{
			return listContent;
		}
