...
import kr.co.actsone.controls.listClasses.ExAdvancedListBase;
import kr.co.actsone.controls.listClasses.ExAdvancedListBaseContentHolder;
...
/**
 *  The ExAdvancedDataGridBase class is the base class for controls
 *  that display lists of items in multiple columns,
 *  such as the ExAdvancedDataGrid and OLAPDataGrid controls.
 *  It is not used directly in applications.
 *  
 *  @mxml
 *  
 *  <p>The ExAdvancedDataGridBase class inherits all the properties of its parent classes
 ...
 */
public class ExAdvancedDataGridBase extends ExAdvancedListBase implements IFontContextComponent
{
     ...
     *
     *  @see kr.co.actsone.controls.ExAdvancedDataGrid#selectedCells
     *  
     *  @langversion 3.0
     *  @playerversion Flash 9
     *  @playerversion AIR 1.1
     ...
     *  @productversion Flex 3
     */
    public function ExAdvancedDataGridBase()
    {
        super();
     ...
     *  @productversion Flex 3
     */
    mx_internal var listSubContent:ExAdvancedListBaseContentHolder;
     ...
     *  Like <code>ExAdvancedListBase.rowMap</code>, this property contains 
     ...
     *  Array of Columns
     */
    protected var _columns:Array; /* of ExAdvancedDataGridColumns */

    /**
     *  An Array of ExAdvancedDataGridHeaderInfo instances for all columns
     *  in the control.
     *
    ...
    /**
     *  An Array of ExAdvancedDataGridHeaderRenderer instances that 
     *  define the header item renderers for the displayable columns.
     *
     *  @see kr.co.actsone.controls.advancedDataGridClasses.ExAdvancedDataGridHeaderRenderer
     *  
     *  @langversion 3.0
     *  @playerversion Flash 9
     *  @playerversion AIR 1.1
    ...
    */
     ...
     *  
     *  If the <code>headerWordWrap</code> property is set in ExAdvancedDataGridColumn,
     ...
     *
     *  @default ExAdvancedDataGridHeaderRenderer
     *  
     *  @langversion 3.0
     *  @playerversion Flash 9
     *  @playerversion AIR 1.1
     ...
     *   and <code>column</code> is the ExAdvancedDataGridColumn instance with which the item renderer is associated.</p>
     *
	...
	//Duong modify
	protected var moveFocusHeaderByKeyboard:Boolean = false;
     ...
     *  @private
     */
	public var mergeColumns:Boolean=false;
    ...
    override protected function finishKeySelection():void
    {
		...
		//Duong modify to reset focus to get dataField of column
		if(_selectedColIndex >= 0 && caretIndex >= 0 && caretIndex - verticalScrollPosition >=0)
		{
			var itemRenderer:IListItemRenderer = listItems[caretIndex - verticalScrollPosition][_selectedColIndex-horizontalScrollPosition];
			if(itemRenderer && (itemRenderer as ExUIComponent))
				 (itemRenderer as ExUIComponent).setFocus();
		}
    }
    ...
    override protected function createChildren():void
    {
        ...
        if (!listSubContent)
        {
            listSubContent = new ExAdvancedListBaseContentHolder(this);
        ...
        if(!headerItems[0] || !headerItems[0][0] || (top < headerItems[0][0].y + headerItems[0][0].height)) 
        {
            ...
            while (/* xx < right && */ currentColNum < optimumColumns.length)
            {
                ...
                else if (headerItems[currentRowNum][currentColNum])
                {
                    ...
                    addHeaderToFreeItemRenderers(freeHeaderItem, freeHeaderItem.data as ExAdvancedDataGridColumn);
                }
     ...
     *  @private
     */
    protected function addHeaderToFreeItemRenderers(item:IListItemRenderer, c:ExAdvancedDataGridColumn):void
    {       
        DisplayObject(item).visible = false;
    ...
    /**
     *  Creates a new ExAdvancedDataGridListData instance and populates the fields based on
     ...
     *  
     *  @return A newly constructed ExAdvancedDataGridListData object.
     *  
     *  @langversion 3.0
     *  @playerversion Flash 9
     *  @playerversion AIR 1.1
                                    ...
                                    rowNum:int, columnNum:int, column:ExAdvancedDataGridColumn):BaseListData
    {
        ...
        {
            return new ExAdvancedDataGridListData((column.headerText != null) ? column.headerText : column.dataField, 
                                                column.dataField, columnNum, uid, this, rowNum);
        }
        else
        { 
            var label:String = column.itemToLabel(data);
            return new ExAdvancedDataGridListData(label, column.dataField, 
     ...
     *  the ExAdvancedDataGridBase already knows the height of
     ...
     *  @productversion Flex 3
     */
    mx_internal function columnItemRenderer(c:ExAdvancedDataGridColumn, forHeader:Boolean, itemData:Object):IListItemRenderer
    {
     ...
     *  @productversion Flex 3
     */
    mx_internal function columnItemRendererFactory(c:ExAdvancedDataGridColumn, forHeader:Boolean, itemData:Object):IFactory
    {
     ...
     *  @productversion Flex 3
     */
    mx_internal function columnHeaderWordWrap(c:ExAdvancedDataGridColumn):Boolean
    {
     ...
     *  @productversion Flex 3
     */
    mx_internal function columnWordWrap(c:ExAdvancedDataGridColumn):Boolean
    {
        if (c.wordWrap == true)
            return true;
        if (c.wordWrap == false)
    ...
    private function createRow(left:Number,top:Number,right:Number,bottom:Number,more:Boolean):void
    {
        ...
        if (itemPending)
        {
            ...
            for (i = 0; i < n; i++)
            {
                obj[ExAdvancedDataGridColumn(optimumColumns[i]).dataField] = "Data Loading";
            }
        ...
        if (listItems[currentRowNum])
        {
			...
			if(n > 0)
				lastColumn = ExAdvancedDataGridColumn(optimumColumns[n -1]);
            for (i = 0; i < n; i++)
            {				
				//Duong modify here to support property fixedLastColumn
				if(i == n - 1 && checkFixedLastColumnWithHScrollBar() && lastColumn)
				{
					listItems[currentRowNum][i].setActualSize(
						lastColumn.width, itemHeight);
				}
				else
				{
	                listItems[currentRowNum][i].setActualSize(
     ...
     *  @productversion Flex 3
     */
    protected function getHeaderRenderer(c:ExAdvancedDataGridColumn):IListItemRenderer
    {
     ...
     *  @productversion Flex 3
     */
    mx_internal function getRenderer(c:ExAdvancedDataGridColumn, itemData:Object, forDragProxy:Boolean = false ):IListItemRenderer
    {
        var renderer:IListItemRenderer;
    ...
    protected function setupRenderer(itemData:Object,uid:String,insertItems:Boolean = false):void
    {
        ...
        while (currentColNum < optimumColumns.length)
        {
            ...
            rowData = ExAdvancedDataGridListData(makeListData(itemData, uid, currentRowNum, c.colNum, c));
            rowMap[item.name] = rowData;
            if (item is IDropInListItemRenderer)
            {
            ...
            item.data = itemData;
            item.visible = verifyVisibleItemRenderer(item);		//Duong add here to prevent item.visible = true in case column is comborenderer
	...
	/////////////////////////////////////////////////////////////////////////////////////////////
	//Duong add
	protected function verifyVisibleItemRenderer(item:IListItemRenderer):Boolean
	{
		return true;
	}
     ...
     *  @private
     */
    protected function addRendererToContentArea(item:IListItemRenderer, column:ExAdvancedDataGridColumn):void
    {
	...
	/**
	 *  @public
	 *  true allow to display blank column
	 *  false does not display blank column
	 *  Duong add
	 */
	public var fixedLastColumn:Boolean = false;
	
	/**
	 *  @public
	 *  true allow to resize blank column
	 *  false does not resize blank column
	 *  Duong add
	 */
	public var allowResizeLastColumn:Boolean = false;
	
	/**
	 *  @public
	 *  save columnIndex	(visible and invisible column index )
	 *  Duong add
	 */
	public var _selectedColIndex:int=-1;
	
	/**
	 *  @public
	 *  save rowIndex	(visible and invisible row index)
	 *  Duong add
	 */
	public var _selectedRowIndex:int=-1;
	
	/**
	 *  @protected
	 *  verify the blank column is displayed or not
	 * Duong add
	 * 
	 */
	protected function checkFixedLastColumnWithHScrollBar():Boolean
	{
		return fixedLastColumn;
	}
}
