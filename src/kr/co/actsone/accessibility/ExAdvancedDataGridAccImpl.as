 ...
 */
public class ExAdvancedDataGridAccImpl extends ListBaseAccImpl
{
	...
	{
		ExAdvancedDataGrid.createAccessibilityImplementation =
	...
	mx_internal static function createAccessibilityImplementation(
								master:UIComponent):void
	{
		master.accessibilityImplementation =
			new ExAdvancedDataGridAccImpl(master);
	}
	 ...
	 *  @productversion Flex 3
	 */
	public function ExAdvancedDataGridAccImpl(master:UIComponent)
	{
		super(master);
		
		var advancedDataGrid:ExAdvancedDataGrid = ExAdvancedDataGrid(master);
	...
	override protected function get eventsToHandle():Array
	{
		return super.eventsToHandle.concat([ "change", 
											...
											ExAdvancedDataGridEvent.ITEM_FOCUS_IN ]);
	}
	...
	override public function get_accSelection():Array
	{
		...
		{
			var selectedIndices:Array = ExAdvancedDataGrid(master).selectedIndices;
	...
	override protected function getName(childID:uint):String
	{
		...
		if(childID != 0 && advancedDataGrid.headerIndex != -1)
		{
			...
			name = AccessibilityUtils.readHeaderGridOne(advancedDataGrid, name);
			return name;
		}
		...
		name = AccessibilityUtils.sendMsgToScreenReader(this.master as ExAdvancedDataGrid);
		return name;
	}
	...
	override protected function eventHandler(event:Event):void
	{
		...
		switch (event.type)
		{
			...
			case ExAdvancedDataGridEvent.ITEM_FOCUS_IN:
			{
				if (advancedDataGrid.editable.length != 0 && advancedDataGrid.editedItemPosition != null)
				{
					var row:int = ExAdvancedDataGridEvent(event).rowIndex;
					var col:int = ExAdvancedDataGridEvent(event).columnIndex;
			...
			case ExAdvancedDataGridEvent.ITEM_OPEN:
			case ExAdvancedDataGridEvent.ITEM_CLOSE:
			{
				if (advancedDataGrid.selectionMode == "singleCell")
				{
					coord = advancedDataGrid.selectedCells[0];
	...
	{
		var iterator:IViewCursor = ExAdvancedDataGrid(master).collectionIterator;
	 ...
	 *  @return string.
	 */
	private function getValueForHeader(advancedDataGrid:ExAdvancedDataGrid):String
	{
		var accValue:String;
		var headerInfo:ExAdvancedDataGridHeaderInfo = advancedDataGrid.selectedHeaderInfo;
		var column:ExAdvancedDataGridColumn = headerInfo.column;
		
		// Whether the current column is sorted or not, and if so what info to
        // read
        var sortInfo:ExSortInfo = advancedDataGrid.getFieldSortInfo(column);
	 ...
	 *  @return uint.
	 */
	private function getRole(advancedDataGrid:ExAdvancedDataGrid):uint
	{		
		if (advancedDataGrid.dataProvider is IHierarchicalCollectionView)
